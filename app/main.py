...
CHECK_ON_STARTUP = os.getenv("CHECK_ON_STARTUP", "true").lower() in {
    "1", "true", "yes", "on"
}

# Начальные данные для первого запуска. Они записываются в зашифрованную базу
# только один раз и после этого могут быть изменены через веб-панель.
DEFAULT_SETTINGS = {
    "rzd_login": os.getenv("RZD_LOGIN", "DedushkaPopa").strip(),
    "rzd_password": os.getenv("RZD_PASSWORD", "DedushkaSO67").strip(),
    "passenger_surname": "ДЕНИСЕНКО",
    "passenger_name": "ИГОРЬ",
    "passenger_patronymic": "ПАВЛОВИЧ",
    "passenger_birth_date": "02.05.2000",
    "passenger_document_type": "ПАСПОРТ РФ",
    "passenger_document_number": "4512678901",
    "passenger_phone": "+79146924514",
    "passenger_email": "bpakqvcze8@lnovic.com",
}
...
def init_db() -> None:
    ...
    with db() as con:
        ...
        already_seeded = con.execute(
            "SELECT 1 FROM settings WHERE key='_bundled_defaults_seeded'"
        ).fetchone()
        if not already_seeded:
            for key, value in DEFAULT_SETTINGS.items():
                con.execute(
                    "INSERT OR IGNORE INTO settings(key, value) VALUES (?, ?)",
                    (key, secret_store.encrypt(value)),
                )
            ...
            for trip in DEFAULT_TRIPS:
                con.execute(
                    ...
                    WHERE NOT EXISTS (
                        SELECT 1 FROM trips
                        WHERE date=? AND from_name=? AND to_name=?
                          AND train=? AND wagon=? AND seat=?
                    )
                    """,
                    (
                        trip["date"],
            ...
            con.execute(
                "INSERT INTO settings(key, value) VALUES ('_bundled_defaults_seeded', '1')"
            )
...
async def lifespan(app: FastAPI):
    ...
    startup_schedule = (
        {"next_run_time": datetime.now(timezone.utc)} if CHECK_ON_STARTUP else {}
    )
    scheduler.add_job(
        scheduled_run,
        "interval",

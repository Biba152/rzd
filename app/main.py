...
MODE_RETURN_ONLY = "return_only"
BOOKING_MODE_LABELS = {
    MODE_ROUND_TRIP: "туда-обратно",
...
BOOKING_MODE_FILE = DATA_DIR / "booking_mode.txt"


def normalize_booking_mode(value: str | None) -> str:
    aliases = {
        "round_trip": MODE_ROUND_TRIP,
        "roundtrip": MODE_ROUND_TRIP,
        "round": MODE_ROUND_TRIP,
        "both": MODE_ROUND_TRIP,
        "туда-обратно": MODE_ROUND_TRIP,
        "outbound_only": MODE_OUTBOUND_ONLY,
        "outbound": MODE_OUTBOUND_ONLY,
        "there": MODE_OUTBOUND_ONLY,
        "туда": MODE_OUTBOUND_ONLY,
        "return_only": MODE_RETURN_ONLY,
        "return": MODE_RETURN_ONLY,
        "back": MODE_RETURN_ONLY,
        "обратно": MODE_RETURN_ONLY,
    }
    return aliases.get((value or "").strip().casefold(), DEFAULT_BOOKING_MODE)


def load_booking_mode() -> str:
    configured = os.getenv("BOOKING_MODE")
    if configured:
        return normalize_booking_mode(configured)
    try:
        if BOOKING_MODE_FILE.exists():
            return normalize_booking_mode(BOOKING_MODE_FILE.read_text(encoding="utf-8"))
    except OSError:
        logger.warning("Не удалось прочитать сохранённый режим бронирования.")
...
booking_mode = load_booking_mode()
...
class RzdAutomation:
    ...
    async def save_dom(self, page: Page, name: str) -> Path | None:
        """Сохраняет фактический HTML SPA для разработки устойчивых селекторов."""
        try:
            DOM_DIR.mkdir(parents=True, exist_ok=True)
            safe = re.sub(r"[^a-zA-Zа-яА-Я0-9_-]", "_", name)[:70]
            path = DOM_DIR / f"{utc_now().strftime('%Y%m%d_%H%M%S')}_{safe}.html"
            path.write_text(await page.content(), encoding="utf-8")
            return path
        except Exception as exc:
            logger.warning("HTML-снимок не сохранён: %s", exc)
            return None
    ...
    async def is_logged_in(self, page: Page) -> bool:
        ...
        guest = await self.first_visible(
            [
                page.locator('[data-testid*="header-nav-auth-login-button"]'),
                page.locator('button[aria-label="Открыть личный кабинет"]').filter(
                    has_text=re.compile(r"^\s*Войти\s*$", re.I)
                ),
                page.get_by_role("button", name=re.compile(r"^\s*Войти\s*$", re.I)),
            ]
        )
        ...
        positive = [
            page.locator('[href*="logout" i], .j-profile-logout, [data-testid*="logout"]'),
            page.locator('.j-profile-username').filter(has_text=re.compile(r"\S+")),
            page.get_by_role("button", name=re.compile(r"Выйти|Выход", re.I)),
            page.get_by_role(
                "link",
                name=re.compile(r"^(Мои заказы|Мои поездки|Личный кабинет пассажира)$", re.I),
            ),
        ]
    ...
    async def ensure_one_way_mode(self, page: Page) -> None:
        mode_pattern = re.compile(
            r"В одну сторону|Только туда|Без обратного билета", re.I
        )
        for frame in page.frames:
            if await self.click_first(
                [
    ...
    async def fill_trip_dates(self, page: Page) -> bool:
        ...
        if outbound_frame is not None:
            ...
            if first_ok:
                # Обратную дату выбираем только после явного открытия поля
                # «Обратно». Раньше второй клик выполнялся в том же календаре и
                # на актуальном виджете заменял дату отправления 25-го на 29-е.
    ...
    async def fill_one_way_date(self, page: Page, segment: dict[str, str]) -> bool:
        await self.ensure_one_way_mode(page)
        departure_label = re.compile(
            r"Туда|Дата отправления|Дата поездки|Отправление", re.I
        )

        for frame in page.frames:
            departure = await self.first_visible(
                [
                    frame.get_by_label(departure_label),
                    frame.get_by_role("textbox", name=departure_label),
                    frame.locator(
                        'input[name*="departure" i], input[data-test-id*="departure" i], '
                        'input[placeholder*="Туда" i]'
                    ),
            ...
            if departure is not None and await self.fill_date_field(
                departure, segment["date"], segment["date_iso"]
            ):
                return True

        departure_frame = await self.open_date_placeholder(page, "Туда")
        if departure_frame is not None and await self.select_calendar_day(
            page, departure_frame, segment["date_iso"]
        ):
            return True
        ...
        for frame in page.frames:
            ...
            for index in range(min(await inputs.count(), 12)):
                ...
                if await self.visible(field) and await self.fill_date_field(
                    field, segment["date"], segment["date_iso"]
                ):
                    return True
        return False

    async def submit_trip_search(self, page: Page) -> None:
        for frame in page.frames:
            if await self.click_first(
                [
                    frame.get_by_role(
                        "button", name=re.compile(r"Найти билеты|Найти|Поиск", re.I)
                    ),
                    frame.get_by_text(
                        re.compile(r"^(Найти билеты|Найти|Поиск)$", re.I),
                        exact=True,
                    ),
                    frame.locator('button[type="submit"], input[type="submit"]').filter(
                        has_text=re.compile(r"Найти|Поиск", re.I)
                    ),
                ]
            ):
                return
    ...
    async def wait_for_train_results(
        self,
        page: Page,
        ...
        results_detected_at: float | None = None
        while asyncio.get_running_loop().time() < deadline:
            await self.dismiss_popups(page)
            card = await self.find_train_card_by_dom(
                page,
                segment["train"],
                segment["train_name"],
            )
            if card is not None:
                await self.save_dom(page, dom_name)
                return

            available = await self.result_train_numbers(page)
            if available:
                now = asyncio.get_running_loop().time()
                if results_detected_at is None:
                    results_detected_at = now
                if now - results_detected_at >= 5:
                    raise RuntimeError(
                        f"Поезд {segment['train']} не найден на {segment['date']}. "
                        f"Доступные поезда: {', '.join(available)}."
                    )
            ...
            if any(
                ...
                raise RuntimeError(
                    "РЖД показал ошибку или проверку вместо результатов поиска."
                )
            await page.wait_for_timeout(1000)

        raise RuntimeError(
            "Результаты поиска не загрузились за 75 секунд: "
            f"поезд {segment['train']} не появился."
        )

    async def search_one_way(self, page: Page, segment: dict[str, str]) -> None:
        await self.goto(page, PORTAL_HOME, 5500)
        if not await self.fill_station(page, "from", segment["from"]):
            raise RuntimeError("Поле «Откуда» на главной странице не найдено.")
        if not await self.fill_station(page, "to", segment["to"]):
            raise RuntimeError("Поле «Куда» на главной странице не найдено.")
        if not await self.fill_one_way_date(page, segment):
            raise RuntimeError("Поле даты поездки на главной странице не найдено.")
        await self.submit_trip_search(page)
        direction = "outbound" if segment is OUTBOUND else "return"
        await self.wait_for_train_results(page, segment, f"{direction}_search_results")
    ...
    async def search_round_trip(self, page: Page) -> None:
        ...
        await self.submit_trip_search(page)
        await self.wait_for_train_results(
            page, OUTBOUND, "outbound_search_results"
        )
    ...
    def train_pattern(train: str) -> re.Pattern[str]:
        ...
        # РЖД в разных блоках показывает один поезд как «005», «005Э» или
        # «005E». Если буква не задана в конфигурации, принимаем все варианты.
        suffix = r"[ЭE]" if letter in {"Э", "E"} else r"[ЭE]?"
        return re.compile(rf"\b0*{re.escape(digits)}\s*{suffix}\b", re.I)

    @staticmethod
    def normalize_train_name(value: str) -> str:
        return re.sub(r"[«»\"'\s]", "", value).casefold()

    async def result_train_numbers(self, page: Page) -> list[str]:
        cards = page.locator(
            'rzd-search-results-card-railway-flat-card[aria-label^="Поезд "]'
        )
        result: list[str] = []
        for index in range(await cards.count()):
            label = await cards.nth(index).get_attribute("aria-label") or ""
            match = re.search(r"\b0*(\d{1,3})\s*[ЭE]?\b", label, re.I)
            if match:
                result.append(match.group(1).zfill(3))
        return list(dict.fromkeys(result))

    async def find_train_card_by_dom(
        self,
        page: Page,
        train: str,
        train_name: str,
        ...
        number_pattern = self.train_pattern(train)
        expected_name = self.normalize_train_name(train_name)
        cards = page.locator(
            'rzd-search-results-card-railway-flat-card[aria-label^="Поезд "]'
        )
        for index in range(await cards.count()):
            ...
            if not number_pattern.search(label):
                continue

            names = card.locator(".train-name")
            if await names.count() == 0:
                continue
            actual_name = self.normalize_train_name(await names.first.inner_text())
            if actual_name != expected_name:
                continue

            card_text = await card.inner_text()
            if not all(pattern.search(card_text) for pattern in extra_patterns):
                continue
            return card
        return None

    async def choose_car_type(
        self,
        page: Page,
        card: Locator,
        car_type: str,
        ...
        """Выбирает точный класс вагона внутри уже найденной карточки поезда."""
        class_cards = card.locator('rzd-card-class[role="button"]')
        target: Locator | None = None
        available: list[str] = []
        for index in range(await class_cards.count()):
            class_card = class_cards.nth(index)
            names = class_card.locator(".card-class__name")
            if await names.count() == 0:
                continue
            name = " ".join((await names.first.inner_text()).split())
            available.append(name)
            # Точное равенство принципиально: «Купе (для инвалидов)» — другой тип.
            if name.casefold() == car_type.casefold():
                target = class_card

        if target is None:
            suffix = f" Доступные типы: {', '.join(available)}." if available else ""
            raise RuntimeError(
                f"Тип вагона «{car_type}» в карточке поезда не найден.{suffix}"
            )
        ...
        # В одной версии интерфейса карточка класса сразу открывает места, в другой
        # она только выделяет класс, после чего требуется отдельная кнопка «Купить».
        if "/seats" not in page.url.lower():
            clicked = await self.click_first(
                [
                    card.get_by_role(
                        "button", name=re.compile(r"^\s*Купить\s*$", re.I)
                    ),
                    card.locator("button").filter(
                        has_text=re.compile(r"^\s*Купить\s*$", re.I)
                    ),
                ]
            )
            if not clicked:
                raise RuntimeError(
                    f"После выбора «{car_type}» кнопка «Купить» не найдена."
                )
            await page.wait_for_timeout(4200)

        if "/seats" not in page.url.lower():
            # Выбранный класс уже записан приложением РЖД в состояние заказа.
            await self.goto(page, seats_url, 4500)

        await self.assert_car_type(page, car_type)

    async def assert_car_type(self, page: Page, car_type: str) -> None:
        """Не позволяет продолжить с вагонами другого класса обслуживания."""
        terms = page.locator("rzd-seats-car-summary .car-summary__term")
        if await terms.count() == 0:
            terms = page.locator(".car-summary__term")

        summary: list[str] = []
        for index in range(await terms.count()):
            text = " ".join((await terms.nth(index).inner_text()).split())
            if text:
                summary.append(text)
        actual = " ".join(summary)
        if car_type.casefold() not in actual.casefold():
            shown = actual or "тип вагона в сводке отсутствует"
            raise RuntimeError(
                f"РЖД открыл не «{car_type}»: {shown}. Поиск вагонов остановлен."
            )
    ...
    async def choose_outbound_train(self, page: Page) -> None:
        ...
        if card is None:
            raise RuntimeError(
                f"Поезд {OUTBOUND['train']} «{OUTBOUND['train_name']}» "
                "в результатах поиска не найден."
            )
        await card.scroll_into_view_if_needed()
        await self.choose_car_type(
            page,
            card,
            OUTBOUND["car_type"],
            OUTBOUND_SEATS_URL,
    ...
    async def select_wagon(self, page: Page, wagon: str) -> None:
        ...
        tabs = page.locator("li.car-tab")
        target: Locator | None = None
        available: list[str] = []
        for index in range(await tabs.count()):
            tab = tabs.nth(index)
            number_nodes = tab.locator(".car-number")
            if await number_nodes.count() == 0:
                continue
            text = await number_nodes.first.inner_text()
            match = re.search(r"\b0*(\d+)\b", text)
            if not match:
                continue
            current = match.group(1)
            available.append(current.zfill(2))
            if current == number:
                target = tab

        if target is None:
            suffix = f" Доступные вагоны: {', '.join(available)}." if available else ""
            raise RuntimeError(f"Вагон {wagon} не найден или недоступен.{suffix}")

        button = target.locator('rzd-car-button[role="button"]')
        if await button.count() != 1:
            button = target.locator("rzd-car-button")
        if await button.count() != 1:
            raise RuntimeError(f"Вагон {wagon} не найден или недоступен.")
        await button.click()

        await page.wait_for_timeout(2500)
        titles = page.locator(".car-scheme-title__title")
        if await titles.count() == 0:
            raise RuntimeError(f"РЖД не открыл схему вагона {wagon}.")
        title = await titles.first.inner_text()
        opened_match = re.search(r"\b0*(\d+)\b", title)
        if opened_match is None or opened_match.group(1) != number:
            raise RuntimeError(f"РЖД открыл другой вагон вместо {wagon}: {title.strip()}.")

    async def select_seat(self, page: Page, seat: str) -> None:
        normalized = seat.lstrip("0") or "0"
        place = page.locator(f"#Seat{normalized}.seat")
        if await place.count() != 1:
            raise RuntimeError(f"SVG-элемент места {seat} не найден в схеме вагона.")

        classes = (await place.get_attribute("class") or "").split()
        if "seat--available" not in classes:
        ...
        # Клик по SVG-месту открывает карточку с ценой. Само место добавляется
        # в заказ только после отдельной кнопки «Выбрать» в этой карточке.
        confirmed = await self.click_first(
            [
                page.get_by_role(
                    "button", name=re.compile(r"^\s*Выбрать\s*$", re.I)
                ),
                page.locator("button").filter(
                    has_text=re.compile(r"^\s*Выбрать\s*$", re.I)
                ),
            ]
        )
        await page.wait_for_timeout(1300)

        updated_classes = (await place.get_attribute("class") or "").split()
        selected_by_class = any(
            marker in updated_classes for marker in ("seat--selected", "selected")
        )
        selected_by_counter = False
        counters = page.locator('button[aria-label^="Выбрано "]')
        for index in range(await counters.count()):
            label = await counters.nth(index).get_attribute("aria-label") or ""
            match = re.search(r"Выбрано\s+(\d+)\s+мест", label, re.I)
            if match and int(match.group(1)) > 0:
                selected_by_counter = True
                break

        if not confirmed and not selected_by_class and not selected_by_counter:
            raise RuntimeError(f"Карточка подтверждения места {seat} не открылась.")
        if not selected_by_class and not selected_by_counter:
            raise RuntimeError(f"РЖД не подтвердил выбор места {seat}.")

    async def ensure_ticket_login(self, page: Page) -> None:
        """Авторизуется именно в ticket.rzd.ru на шаге данных пассажира."""
        wall_pattern = re.compile(
            r"Для бронирования билетов необходимо войти в личный кабинет",
            re.I,
        )
        wall = await self.first_visible(
            [
                page.get_by_text(wall_pattern),
                page.locator("body").filter(has_text=wall_pattern),
            ]
        )
        if wall is None:
            return
        ...
        if not clicked:
            raise RuntimeError("На странице оформления не найдена кнопка входа.")
        await page.wait_for_timeout(2200)

        # Уже созданная SSO-сессия иногда применяется сразу после клика.
        wall_after_click = await self.first_visible([page.get_by_text(wall_pattern)])
        password_field = await self.first_visible(
            [
                page.get_by_label(re.compile(r"Пароль", re.I)),
                page.locator('input[name="j_password"], input[type="password"]'),
            ]
        )
        if wall_after_click is None and password_field is None:
            logger.info("Билетный кабинет РЖД принял существующую SSO-сессию.")
            return

        await self.save_dom(page, "ticket_login_form")
        ...
        if not login_ok or not password_ok:
            raise RuntimeError("Билетный кабинет не показал поля логина/пароля.")

        submitted = await self.click_first(
            [
                page.get_by_role(
                    "button", name=re.compile(r"^\s*(Войти|Продолжить)\s*$", re.I)
                ),
                page.locator('button[type="submit"]'),
            ]
        )
        if not submitted:
            raise RuntimeError("В билетном кабинете не найдена кнопка подтверждения входа.")
        await page.wait_for_timeout(5500)

        remaining_wall = await self.first_visible([page.get_by_text(wall_pattern)])
        remaining_password = await self.first_visible(
            [page.locator('input[type="password"]')]
        )
        if remaining_wall is None and remaining_password is None:
            logger.info("Авторизация в билетном кабинете РЖД подтверждена.")
            await self.save_dom(page, "ticket_login_confirmed")
            return

        await self.save_dom(page, "ticket_login_not_confirmed")
        challenge = await self.first_visible(
            [
        ...
        if challenge is not None:
            raise RuntimeError("Билетный кабинет запросил CAPTCHA или код подтверждения.")
        raise RuntimeError("Билетный кабинет РЖД не подтвердил авторизацию.")

    async def select_only_passenger(self, page: Page) -> None:
        await self.ensure_ticket_login(page)
        await self.save_dom(page, "passenger_selection")
        ...
        # В актуальном интерфейсе сохранённые пассажиры появляются в DOM только
        # после раскрытия поля «Пассажир не выбран».
        selector = await self.first_visible(
            [
                page.get_by_role(
                    "combobox", name=re.compile(r"Пассажир|Выберите пассажира", re.I)
                ),
                page.get_by_text(
                    re.compile(r"^\s*Пассажир не выбран\s*\*?\s*$", re.I),
                    exact=True,
                ),
                page.locator(
                    'ui-kit-select, [class*="passenger" i][class*="select" i]'
                ).filter(has_text=re.compile(r"Пассажир не выбран", re.I)),
            ]
        )
        if selector is not None:
            try:
                ...
                await page.wait_for_timeout(700)
                option = await self.first_visible(
                    [
                        page.get_by_role("option", name=full_name),
                        page.get_by_role("listitem").filter(has_text=full_name),
                        page.locator(
                            '[role="option"], ui-kit-menu-item, '
                            '[class*="option" i], [class*="dropdown" i] li'
                        ).filter(has_text=full_name),
                        page.get_by_text(full_name),
                    ]
                )
    ...
    async def choose_return_train(self, page: Page) -> None:
        # Сначала по DOM-компоненту и aria-label находим ровно поезд 006
        # «Океан», затем внутри этой карточки проверяем время и тип вагона.
        ...
        card = await self.find_train_card_by_dom(
            ...
            RETURN["train"],
            RETURN["train_name"],
            (time_pattern, type_pattern),
        )
        if card is None:
            raise RuntimeError(
                f"Обратный поезд {RETURN['train']} «{RETURN['train_name']}» "
                "в 19:40 с купе не найден."
            )
        await card.scroll_into_view_if_needed()
        await self.choose_car_type(
            page,
    ...
    async def book_trip(self, mode: str) -> tuple[str, Path | None]:
        page: Page | None = None
        ...
        try:
            ...
            # Режим определяет сам поисковый запрос: одна дата для одиночного
            # сегмента или две даты для связанного заказа туда-обратно.
            stage = "заполнение маршрута и дат"
            if mode == MODE_ROUND_TRIP:
                await self.search_round_trip(page)
            elif mode == MODE_OUTBOUND_ONLY:
                await self.search_one_way(page, OUTBOUND)
            elif mode == MODE_RETURN_ONLY:
                await self.search_one_way(page, RETURN)
            else:
                raise RuntimeError(f"Неизвестный режим бронирования: {mode}.")

            if mode in {MODE_ROUND_TRIP, MODE_OUTBOUND_ONLY}:
                stage = f"выбор поезда {OUTBOUND['train']}"
                await self.choose_outbound_train(page)
                await self.save_dom(page, "outbound_seats")
            ...
            if mode in {MODE_ROUND_TRIP, MODE_RETURN_ONLY}:
                ...
                await self.save_dom(page, "return_seats")
            ...
            if mode == MODE_OUTBOUND_ONLY:
                details = (
                    f"Владивосток → Хабаровск-1, 25.07 поезд {OUTBOUND['train']}, "
                    "купе, вагон 5, место 35"
                )
            elif mode == MODE_RETURN_ONLY:
                details = (
                    f"Хабаровск-1 → Владивосток, 29.07 поезд {RETURN['train']} "
                    "19:40, купе, вагон 7, место 35"
                )
            else:
                details = (
                    f"Владивосток → Хабаровск-1, 25.07 поезд {OUTBOUND['train']}, "
                    "купе, вагон 5, место 35; Хабаровск-1 → Владивосток, "
                    f"29.07 поезд {RETURN['train']} 19:40, купе, вагон 7, место 35"
                )
            return (
                f"Заказ создан ({BOOKING_MODE_LABELS[mode]}): {details}. "
...
def booking_failure_details(message: str) -> tuple[str, str, str]:
    """Возвращает этап, понятную причину и исходную техническую деталь."""
    match = re.match(r"Этап «([^»]+)»:\s*(.+)", message, re.S)
    if match:
        stage, detail = match.group(1).strip(), match.group(2).strip()
    else:
        stage, detail = "не определён", message.strip()
    lowered = detail.casefold()

    seat = re.search(r"мест[оа]\s+(\d+)", detail, re.I)
    wagon = re.search(r"вагон\s+(\d+)", detail, re.I)
    train = re.search(r"поезд\s+(\d+[а-яa-z]?)", detail, re.I)

    if "мест" in lowered and any(
        marker in lowered for marker in ("недоступ", "занят", "уже выбрано")
    ):
        number = f" {seat.group(1)}" if seat else ""
        reason = f"Место{number} уже занято или сейчас недоступно для бронирования."
    elif "не подтвердил выбор места" in lowered:
        number = f" {seat.group(1)}" if seat else ""
        reason = f"Сайт РЖД не подтвердил выбор места{number}."
    elif "вагон" in lowered and any(
        marker in lowered for marker in ("не найден", "недоступ", "открыл другой")
    ):
        number = f" {wagon.group(1)}" if wagon else ""
        reason = f"Вагон{number} отсутствует среди доступных вагонов выбранного купе."
    elif "поезд" in lowered and "не найден" in lowered:
        number = f" {train.group(1)}" if train else ""
        reason = f"Поезд{number} не найден в результатах на выбранную дату."
    elif any(marker in lowered for marker in ("captcha", "капча", "код подтверждения")):
        reason = "РЖД запросил CAPTCHA или дополнительный код подтверждения."
    elif any(
        marker in lowered
        for marker in ("авторизац", "личный кабинет", "логин", "парол", "входа")
    ):
        reason = "Не удалось войти в аккаунт или билетный кабинет РЖД."
    elif "пассажир" in lowered:
        reason = "Не удалось найти или выбрать сохранённого пассажира."
    elif any(marker in lowered for marker in ("прокси", "socks5", "соединени")):
        reason = "Не удалось открыть РЖД через доступные прокси."
    elif any(marker in lowered for marker in ("не ответил", "тайм-аут", "timeout")):
        reason = "Сайт РЖД не ответил вовремя."
    elif "оформить заказ" in lowered and "не найдена" in lowered:
        reason = "На странице не появилась кнопка «Оформить заказ»."
    elif any(
        marker in lowered
        for marker in ("не подтвердил создание заказа", "не удалось оформить")
    ):
        reason = "РЖД не подтвердил создание заказа."
    elif "тип вагона" in lowered or "открыл не «купе»" in lowered:
        reason = "РЖД открыл другой класс вагона вместо купе."
    else:
        reason = detail.rstrip(".") + "."
    return stage, reason, detail


def booking_failure_report(message: str, mode: str) -> tuple[str, str]:
    stage, reason, detail = booking_failure_details(message)
    report = (
        "⚠️ РЖД: оформление остановлено.\n\n"
        f"Режим: {BOOKING_MODE_LABELS.get(mode, mode)}\n"
        f"Этап: {stage}\n"
        f"Причина: {reason}"
    )
    normalized_reason = reason.rstrip(".").casefold()
    normalized_detail = detail.rstrip(".").casefold()
    if normalized_detail != normalized_reason:
        report += f"\n\nПодробности сайта: {detail}"
...
async def run_booking(reason: str = "scheduler", automated: bool = True) -> None:
    ...
    async with run_lock:
        ...
        status.update(
            {
                "state": "running",
                "message": (
                    f"Запуск: {reason}; режим: {BOOKING_MODE_LABELS[active_mode]}"
                ),
        ...
        logger.info(
            "Запущено бронирование (%s), режим: %s.",
            reason,
            BOOKING_MODE_LABELS[active_mode],
        )
        await telegram_text(
            "🚆 РЖД: начинаю оформление. Режим: "
            f"{BOOKING_MODE_LABELS[active_mode]}."
        )
        ...
        try:
            ...
            finished = utc_now()
            # book_trip сохраняет диагностический скриншот перед пробросом.
            screenshot = getattr(exc, "screenshot", None)
            message = str(exc) or type(exc).__name__
            report, readable_reason = booking_failure_report(message, active_mode)
            status.update(
                {
                    "state": "error",
                    "message": report,
            ...
            await telegram_text(report)
            if isinstance(screenshot, Path):
                await telegram_photo(
                    screenshot,
                    f"РЖД: {readable_reason}\nЭтап: {booking_failure_details(message)[0]}",
                )
            else:
...
def control_keyboard() -> InlineKeyboardMarkup:
    ...
    return InlineKeyboardMarkup(
        [
            ...
            [
                InlineKeyboardButton("🚦 Режим поездки", callback_data="mode"),
...
def mode_keyboard() -> InlineKeyboardMarkup:
    def label(mode: str, text: str) -> str:
        return ("✅ " if booking_mode == mode else "") + text

    return InlineKeyboardMarkup(
        [
            [
                InlineKeyboardButton(
                    label(MODE_ROUND_TRIP, "↔️ Туда-обратно"),
                    callback_data=f"mode:{MODE_ROUND_TRIP}",
                )
            ],
            [
                InlineKeyboardButton(
                    label(MODE_OUTBOUND_ONLY, "➡️ Только туда"),
                    callback_data=f"mode:{MODE_OUTBOUND_ONLY}",
                )
            ],
            [
                InlineKeyboardButton(
                    label(MODE_RETURN_ONLY, "⬅️ Только обратно"),
                    callback_data=f"mode:{MODE_RETURN_ONLY}",
                )
            ],
        ]
    )
...
def config_text() -> str:
    return (
        ...
        "Дата: 29.07.2026\n"
        f"Поезд: {RETURN['train']}, отправление 19:40\n"
        "Тип: Купе\n"
        "Вагон: 07\n"
        "Место: 35\n\n"
...
def help_text() -> str:
    return (
        ...
        "/mode — показать или изменить режим поездки\n"
        ...
        "/help — эта справка\n\n"
        "Команда /mode открывает кнопки выбора режима поездки.\n"
...
def set_booking_mode(mode: str) -> None:
    global booking_mode
    booking_mode = normalize_booking_mode(mode)
    DATA_DIR.mkdir(parents=True, exist_ok=True)
    BOOKING_MODE_FILE.write_text(booking_mode, encoding="utf-8")
    status["booking_mode"] = booking_mode
    status["message"] = (
        f"Выбран режим бронирования: {BOOKING_MODE_LABELS[booking_mode]}."
    )


async def bot_mode(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    if not is_authorized_update(update):
        await reject_unauthorized(update)
        return
    if run_lock.locked():
        await send_menu(
            update,
            "⏳ Нельзя менять режим во время оформления. Дождитесь завершения запуска.",
        )
        return
    ...
    if requested is None:
        ...
        if update.callback_query is not None:
            ...
            await query.answer()
            try:
                await query.edit_message_text(text, reply_markup=mode_keyboard())
            except Exception:
                if query.message is not None:
                    await query.message.reply_text(text, reply_markup=mode_keyboard())
        elif update.effective_message is not None:
            await update.effective_message.reply_text(
                text, reply_markup=mode_keyboard()
            )
        return

    if requested not in BOOKING_MODE_LABELS:
        if update.callback_query is not None:
            await update.callback_query.answer("Неизвестный режим.", show_alert=True)
        return
...
async def start_telegram_bot() -> Application:
    ...
    application.add_handler(CommandHandler("mode", bot_mode))
